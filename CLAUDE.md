...
- **Hydration mismatch on `/home`.** `components/Header.tsx` renders `<AvatarImage>` only when `session?.user?.email !== undefined`. The session comes from `authClient.useSession()`, which is client-only, so SSR emits just the `<AvatarFallback>CN</AvatarFallback>` and the client renders a different tree. React discards the server HTML for that subtree; in dev it surfaces as the red "1 error" overlay. A `mounted` guard or `suppressHydrationWarning` fixes it.
- **The search box defaults to `exact`, not `fuzzy`.** `app/home/components/SearchComponent.tsx:50` initialises `matchMode` to `"exact"`, while `searchAll`'s parameter default (`app/actions/search.ts:44`) and the URL fallback (`app/results/SearchResults.tsx:23`) are both `"fuzzy"`. Whichever is intended, three places disagree.
- **Fuzzy scores the whole query, not word by word.** `word_similarity(query, col)` against a 0.7 threshold means two-word queries usually fall short even when both words are present: `word_similarity('postgre', title)` = 0.875 on the pg_trgm bookmark, `word_similarity('postgre trigram', title)` = 0.500 on the same row. `loose` at 0.5 does not save it either, since the comparison is `>`. Splitting the query and combining per-word scores is the fix if it ever matters.
- **`createDefaultSettings` and the Prisma schema disagree on defaults.** `app/actions/settings.ts:84` writes `bookmarkDisplay: "Title,Description,Tags,Actions"` and `itemsPerPage: 20`; `prisma/schema.prisma:79` declares `"Title,Description,Tags,Actions,Images"` and `10`. Only the action path runs today (`registerUser` calls it), so the schema defaults are dead — but a row created any other way renders differently. A user with no settings row at all falls back to `SettingsContext`'s `bookmarkDisplay: []`, which renders a card with nothing on it but the URL.
- **Browser verification: done.** `/login`, `/home`, `/results` and the create-bookmark dialog were driven in headless Chrome against a production build (`next build && next start`) with 12 seeded bookmarks, 4 folders and 8 tags. Layout, metadata autofill, fuzzy search and match highlighting all work. The findings above came out of that pass. `/filters` and `/settings` are still unverified in a browser.
...
`python/sql.py` + `python/insert.sql` (SQLite-syntax seed data) and the tracked `.idea/` are gone too. Nothing references them; don't bring them back.
